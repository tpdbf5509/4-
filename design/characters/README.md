...
| 보급소 | supply-tower-sheet.webp | supply-tower.webp · 715×1066 · 197KB | 제목·문구·발밑 그림자 |

서리탑은 흰 머리와 흰 옷이 연한 하늘색 배경과 색이 거의 같습니다.
그래서 색이 아니라 윤곽선을 벽으로 삼아 배경을 흘려 채우는 방식으로 떼어 냈습니다.

보급소는 요청에 따라 주황 계열을 흰빛으로 바꿨습니다. 유일하게 색을 바꾼 캐릭터입니다.
`recolor.py` 가 그 코드입니다. 색상각을 조금씩 돌리면 중간에 초록을 지나가므로,
목표 색으로 통째로 옮긴 그림을 따로 만들어 원본과 RGB에서 섞습니다. 섞는 비율은 채도로 정합니다.
채도 0.16 아래(살갗·흰 프릴)는 그대로 두고 0.36 위(머리·옷·가방)는 온전히 바꿉니다.
지금 쓰는 값은 색상각 35도, 채도 ×0.10, 명도를 남은 만큼의 45% 끌어올린 '순백'입니다.
한 번 #BB88D6(색상각 279도) 보라로도 만들어 봤고, 같은 코드에서 값만 바꾸면 됩니다.
