...
보급소는 요청에 따라 주황 계열을 #BB88D6(색상각 279도)으로 옮겼습니다. 유일하게 색을 바꾼 캐릭터입니다.
`recolor.py` 가 그 코드입니다. 색상각을 조금씩 돌리면 중간에 초록을 지나가므로,
색상각을 통째로 옮긴 그림을 따로 만들어 원본과 RGB에서 섞습니다. 섞는 비율은 채도로 정합니다.
채도 0.24 아래(살갗·흰 프릴)는 그대로 두고 0.42 위(머리·옷·가방)는 온전히 바꿉니다.
